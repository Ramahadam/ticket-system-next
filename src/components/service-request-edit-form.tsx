...
import { updateServiceRequestAction } from '@/app/(staff)/requests/actions';
...
export function ServiceRequestEditForm({
  requestId,
  userId,
  isStaff,
  engineers,
  ...
  const [impact, setImpact] = React.useState(currentImpact ?? '');
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (isStaff) {
        ...
        if (priority !== currentPriority) fd.set('priority', String(priority));
        if ((impact || '') !== (currentImpact ?? '') && impact) fd.set('impact', impact);
      }
      if (note.trim()) fd.set('noteValue', note.trim());
      if (uploadedUrl) fd.set('file', uploadedUrl);
      await updateServiceRequestAction(requestId, fd);
